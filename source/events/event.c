...
/*   Updated: 2020/03/01 15:07:21 by kzina            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	event_2(t_rt *rt, t_move *move)
{
	...
	else if (rt->event.type == SDL_MOUSEBUTTONDOWN)
	{
		if (rt->event.window.windowID == SDL_GetWindowID(rt->window.window))
			mouse_down(rt);
		else if (rt->event.window.windowID == SDL_GetWindowID(rt->win_menu.window))
			m_down_menu(rt);
	}
	//refresh_menu(rt);
}
...
void		events(t_rt *rt)
{
	...
	while (1)
		while (SDL_PollEvent(&rt->event))
		{
			...
			else if (SDLK_RETURN == rt->event.key.keysym.sym && rt->event.type == SDL_KEYDOWN)
				rt->select_obj = -1;
			else if ((SDLK_p == rt->event.key.keysym.sym || SDLK_o == rt->event.key.keysym.sym)
						&& rt->event.type == SDL_KEYDOWN)
				detail_key(rt);
			else if ((SDLK_r == rt->event.key.keysym.sym || SDLK_f == rt->event.key.keysym.sym)
						&& rt->event.type == SDL_KEYDOWN)
				reflection_key(rt);
			else
				event_2(rt, &move);
