...
/*   Updated: 2020/03/05 18:55:59 by kzina            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	dirate_key(t_rt *rt)
{
	...
	if (SDLK_LEFT == rt->event.key.keysym.sym && (tmp = rt->cam.pos.x) < 20000)
	{
		rt->cam.pos.x = rt->cam.pos.x * cosf(ALPHA) + rt->cam.pos.z * (sinf(ALPHA));
		rt->cam.pos.z = tmp * (-sinf(ALPHA)) + rt->cam.pos.z * cosf(ALPHA);
	}
	else if (SDLK_RIGHT == rt->event.key.keysym.sym && (tmp = rt->cam.pos.x) > -20000)
	{
		rt->cam.pos.x = rt->cam.pos.x * cosf(-ALPHA) + rt->cam.pos.z * sinf(-ALPHA);
		rt->cam.pos.z = tmp * (-sinf(-ALPHA)) + rt->cam.pos.z * cosf(-ALPHA);
	}
	else if (SDLK_DOWN == rt->event.key.keysym.sym && (tmp = rt->cam.pos.y) > -20000)
	{
		rt->cam.pos.y = rt->cam.pos.y * cosf(-ALPHA) + rt->cam.pos.z * (-sinf(-ALPHA));
		rt->cam.pos.z = tmp * sinf(-ALPHA) + rt->cam.pos.z * cosf(-ALPHA);
	}
...
{
	if (SDLK_d == rt->event.key.keysym.sym)
	{
...
{
	if (SDLK_w == rt->event.key.keysym.sym || SDLK_s == rt->event.key.keysym.sym || SDLK_a == rt->event.key.keysym.sym || SDLK_d == rt->event.key.keysym.sym
		|| SDLK_KP_PLUS == rt->event.key.keysym.sym || SDLK_KP_MINUS == rt->event.key.keysym.sym)
		move_key(rt, move);
	else if (SDLK_LEFT == rt->event.key.keysym.sym || SDLK_RIGHT == rt->event.key.keysym.sym || SDLK_DOWN == rt->event.key.keysym.sym ||
		SDLK_UP == rt->event.key.keysym.sym || SDLK_PAGEUP == rt->event.key.keysym.sym || SDLK_PAGEDOWN == rt->event.key.keysym.sym)
		dirate_key(rt);
}
